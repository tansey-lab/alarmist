...
def main():
    ...
    logger.info(f"Cell types: {adata.obs[args.cell_type_column].cat.categories.tolist()}")
    ...
    logger.info(f"Matrix sparsity: {(1 - results['patch_lri_matrix'].nnz / np.prod(results['patch_lri_matrix'].shape)) * 100:.2f}%")
    ...
    logger.info("\nFiles created:")
    logger.info(f"  - patch_lri_matrix.npz (sparse matrix)")
    if args.data_thinning != 1.0:
        t = int(args.data_thinning * 100)
        logger.info(f"  - patch_lri_matrix_{t}.npz (thinned sparse matrix)")
    logger.info(f"  - patch_lri_columns.csv (column names)")
    logger.info(f"  - patch_tma_correspondence.csv (patch-TMA mapping)")
    logger.info(f"  - cell_patch_correspondence.csv (cell-patch mapping)")
    logger.info(f"  - analysis_parameters.csv (analysis parameters)")
