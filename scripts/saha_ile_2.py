...
results_dir = "/Users/jiayifan/tansey_lab/alarmist/results/SAHA_ILE"
# lri_results = al.load_patch_lri_results(results_dir)
n_components = 20

# adata = sc.read_h5ad('/Users/jiayifan/tansey_lab/saha_ibd/SAHA_ILE_RNA.h5ad')
# adata.obs['cell_type'] = adata.obs['Insitutype_Broad']
...
for i, info in cell_results['sample_info'].iterrows():
    sample_id = info['sample_id']
    start = info['global_cell_idx_start']
    end = info['global_cell_idx_end'] + 1  # +1 because slice is exclusive
    cell_loadings_by_sample[sample_id] = cell_loadings[start:end, :]
    print(f"{sample_id}: {cell_loadings_by_sample[sample_id].shape}")
