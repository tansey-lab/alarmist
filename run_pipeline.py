...
def main():
    ...
    parser.add_argument(
        "--patch-size", type=float, default=50.0, help="Patch size in μm"
    )
    parser.add_argument("--skip-glm", action="store_true", help="Skip GLM analysis")
    parser.add_argument(
        "--skip-visualization", action="store_true", help="Skip GLM visualization"
    )
    parser.add_argument(
        "--bptf-conda-env",
        default="bptf",
        help="Conda environment for BPTF step (default: bptf)",
    )
    parser.add_argument(
        "--main-conda-env",
        default="tf2.10",
        help="Conda environment for other steps (default: tf2.10)",
    )
    parser.add_argument(
        "--no-conda",
        action="store_true",
        help="Run without conda environment switching",
    )
    ...
    cmd1 = (
        f"python {script_dir}/01_run_patch_lri_analysis.py "
    ...
    cmd2 = (
        f"python {script_dir}/02_bptf_matrix_factorization.py "
    ...
    cmd3 = (
        f"python {script_dir}/03_bptf_visualization.py "
        f"--bptf-dir {bptf_dir} "
    ...
    if not args.skip_glm:
        cmd4 = (
            f"python {script_dir}/04_poisson_glm.py "
            f"--data-file {args.data_file} "
    ...
    if not args.skip_visualization and not args.skip_glm:
        cmd5 = (
            f"python {script_dir}/05_glm_results.py "
            f"--data-file {args.data_file} "
            f"--results-dir {glm_dir} "
            f"--output-dir {plots_dir}/glm_plots"
        )

        if not run_command_with_conda(
            cmd5, "Step 5: GLM Results Visualization", main_env
        ):
